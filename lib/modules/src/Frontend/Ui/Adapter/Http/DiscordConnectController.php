...
final readonly class DiscordConnectController extends WebController
{
    ...
    public function check(): Response
    {
        ...
        if ($linkingToken) {
            return $this->handleLinking($linkingToken, $discordUser);
        }

        return $this->redirect('home_get');
    }

    private function handleLinking(string $token, ResourceOwnerInterface $discordUser): Response
    {
        $session = $this->requestStack->getSession();
        ...
        try {
            ($this->linkSubscriptionUseCase)($linkingToken, $discordUser->getId());
