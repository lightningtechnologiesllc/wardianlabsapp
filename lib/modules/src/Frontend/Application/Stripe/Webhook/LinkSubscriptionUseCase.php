...
final readonly class LinkSubscriptionUseCase
{
    ...
    public function __invoke(AccountLinkingToken $linkingToken, string $discordUserId): void
    {
        ...
        // Assign Discord roles for all guilds
        foreach ($rolesToAssignPerGuild as $guildId => $roleIds) {
            try {
                ...
                $this->logger->error('Failed to assign Discord roles', [
                    'guild_id' => $guildId,
                    'discord_user_id' => $discordUserId,
                    'role_ids' => $roleIds,
                    'error' => $e->getMessage(),
