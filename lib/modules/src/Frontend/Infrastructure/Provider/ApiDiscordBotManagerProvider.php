...
class ApiDiscordBotManagerProvider implements DiscordUserManagerProvider
{
    ...
    public function throwException($code, string $guildId, mixed $roleId, string $userId): void
    {
        switch ($code) {
            ...
            case 50013:
                throw new MissingPermissionsException("Missing permissions to add role $roleId to user $userId in guild $guildId.");
            default:
                throw new Exception("Unknown error occurred with code $code for guild $guildId, role $roleId, and user $userId.");
        }
    }
}
